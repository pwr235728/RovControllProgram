...
class AHRS_Separate():
    ...
    def __init__(selfotX: ):
        self.serial =otX:  serial_port = serial.Serial(IMU_PORT, 115200, stopbits=2, parity=serial.PARITY_NONE)
        self.lock_ratotX: e_of_turn = threading.Lock()
        self.lock_freotX: e_acc = threading.Lock()
        self.lock_angotX: ular_pos = threading.Lock()
        self.logger =otX:  Logger(filename='ahrs',directory='',logtype='info',timestamp='%Y-%m-%d | %H:%M:%S.%f',logformat='[{timestamp}] {logtype}:    {message}',prefix='',postfix='',title='AHRS logger',logexists='append',console=False) 
        self.close_orotX: der = False
otX: 
    def get_message(sotX: elf):
        MID = 0
        data = 0
        ser = self.serial
...
if __name__ == "__main__":
    ...
    imu = AHRS_Separate()
    t = threading.Thread(target=imu.run)
    t.start()
